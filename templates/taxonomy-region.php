...
<div class="result">
 ...
 if (isset($queried_object)) {
     $region = $queried_object->slug;
 } else {
     $region = htmlspecialchars($_COOKIE['region']); // => 'departement'
 }
 ...
 if (isset($_COOKIE['departement'])) {
     $departement = htmlspecialchars($_COOKIE['departement']); // => 'Departement'
 }

 if (isset($_COOKIE['villes'])) {
     $ville = htmlspecialchars($_COOKIE['villes']); // => 'location'
 }
